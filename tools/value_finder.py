...
def recency(v, race_date, rec_date):
    """❌ 古い戦績を平均へ回帰させる減衰。2026-08-16に実測で有害と判明したため無効化した。

    2026-08-15に「3年前のG1成績を満点計上するのはおかしい」という正しさの理由だけで導入したが、
    導入時点の14鞍バックテストでは差が出ず効果は未実証だった。翌8/16に実害が出た:
      中京記念のサトノシャイニング(1番人気)は減衰なしなら地力1位112.8だったのを
      10ヶ月の実戦間隔を理由に11位87.7まで沈め、実際には3着に来た。
    結果が揃った19鞍で再検証すると 減衰なし=複勝率42%/複回収139% に対し
    どの強さの減衰でも 37%/131% と一貫して悪化した。

    教訓: バックテストで差が出ない変更は「正しそう」でも入れない。
    関数は履歴として残すが恒等変換にしてある。
    """
    return v
